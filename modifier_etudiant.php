...
<?php
  ...
  if (isset($_POST['save'])) 
        ...
        if (!empty($_POST["nom"]) && !empty($_POST["prenom"]) && !empty($_POST["datee"]) && !empty($_POST["password"]) && !empty($_POST["email"]) && !empty($_POST["tel"]) && !empty($_POST["code_groupe"])) 
          ...
          $search=$cnx->prepare("UPDATE `etudiant` SET `nom`=:nom,`prenom`=:prenom,`datee`=:datee,`password`=:password,`email`=:email,`tel`=:tel,`code_groupe`=:code_groupe WHERE id_etud=:id");
